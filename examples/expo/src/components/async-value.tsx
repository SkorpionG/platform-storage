...
import type { PlatformStorageError } from "@platform-storage/react-native";
...
export function AsyncValue({ stored }: AsyncValueProps) {
  if (stored.status === STORED_VALUE_STATUS.Loading) return <Pending>reading…</Pending>;
...
export function NotReady({ stored }: AsyncValueProps) {
  ...
  return <Pending>reading…</Pending>;
}
